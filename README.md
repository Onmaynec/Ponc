...
- три уровня сложности компьютера: `Легко`, `Средне`, `Сложно`;
- сохранение выбранной сложности между запусками;
...
Сложность выбирается в стартовом меню и фиксируется на время матча. Настройка сохраняется в `localStorage`; неизвестное или повреждённое значение безопасно заменяется профилем `Средне`.

| Профиль  | Поведение                                                                    |
| -------- | ---------------------------------------------------------------------------- |
| `Легко`  | Медленная ракетка, заметная задержка реакции и большая погрешность наведения |
| `Средне` | Сбалансированный профиль, близкий к поведению версии 1.1.0                   |
| `Сложно` | Высокая скорость, короткая задержка и точное наведение                       |

Профили управляют скоростью ракетки, интервалом реакции, зоной нечувствительности и допустимой ошибкой цели. Они не меняют скорость мяча, победный счёт или другие правила матча.

## Архитектура

```text
src/
├── config/       # централизованная конфигурация размеров, скоростей и AI
...
├── systems/      # AI, физика и нормализация ввода
...
3. `GameCore` управляет состояниями, счётом, выбранной сложностью и событиями.
4. `PhysicsSystem` изменяет только игровую модель и возвращает факт гола.
5. `AISystem` получает снимок модели и формирует команду согласно активному профилю.
...
Core-тесты не используют реальный Canvas, DOM или браузер. Отдельный тест имитирует интервалы обновления 60, 120 и 144 Гц и проверяет эквивалентное движение модели. AI-тесты фиксируют генератор случайных чисел и проверяют профили, задержку реакции и применение скорости к физике.
